...
class DirScanner():
    ...
    def scan(self):
        ...
        Files may exist but have not been sent yet, if so then resend. Also 
        file changes are detected using a checksum, in case of a change they
        are flagged to be resent"""
        newfiles = []
        ...
        for file in os.listdir(self.path):
            ...
            # if the does not existthen add to send list
            if fullpath not in self.filelist:
                ...
                if self.filelist[fullpath].sent == False or self.filelist[fullpath].checksum != checksum:
                    newfiles.append(fullpath)
